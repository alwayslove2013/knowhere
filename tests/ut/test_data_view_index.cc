...
template <typename DataType>
void
EmbListAddTest(const knowhere::DataSetPtr train_ds, const knowhere::DataSetPtr query_ds, const int64_t k,
               const knowhere::MetricType metric, const knowhere::Json& conf, const size_t each_el_len) {
    auto train_ds_list = SplitEmbListDataSet(train_ds, 3, each_el_len);
    auto query = knowhere::ConvertToDataTypeIfNeeded<DataType>(query_ds);
    auto version = knowhere::Version::GetCurrentVersion().VersionNumber();
    ...
    for (size_t i = 0; i < train_ds_list.size(); i++) {
        auto base = knowhere::ConvertToDataTypeIfNeeded<DataType>(train_ds_list[i]);
        if (i == 0) {
            REQUIRE(scann_with_dv_refiner.Build(base, conf, false) == knowhere::Status::success);
        } else {
    ...
    auto knn_gt_ids = knn_gt.value()->GetIds();
    auto knn_gt_dis = knn_gt.value()->GetDistance();
    auto actual_nq = knn_gt.value()->GetRows();
    auto actual_topk = knn_gt.value()->GetDim();

    auto scann_with_dv_refiner_emb_list_results = scann_with_dv_refiner.Search(query, conf, nullptr);
    REQUIRE(scann_with_dv_refiner_emb_list_results.has_value());
    auto res_ids = scann_with_dv_refiner_emb_list_results.value()->GetIds();
    auto res_dis = scann_with_dv_refiner_emb_list_results.value()->GetDistance();
